...
class MessagesState(TypedDict):
    # 定义messages字段，类型为消息序列，Annotated[类型, 附加信息]，使用add_messages处理追加
...
class ParallelToolNode(ToolNode):
    ...
    def __init__(self, tools, max_workers: int | None = None):
        ...
        if max_workers is None:
            self.max_workers = min(tool_count, cpu_limit * 2, 8)
        else:
...
def filter_messages(messages: list) -> list:
    ...
    filtered = [msg for msg in messages if isinstance(msg, (AIMessage, HumanMessage))]
...
# 该函数用于创建一个LLM处理链，加载提示模板并绑定模型，使用缓存避免重复读取文件。
def create_chain(llm_chat, template_file: str, structured_output=None):
    ...
    try:
        ...
        # 第一层：无锁读取缓存（提高性能）
        cache_entry = create_chain.prompt_cache.get(template_file)
        # 检查缓存是否过期
        if cache_entry and cache_entry["last_modified"] == file_mtime:
            prompt_template = cache_entry["template"]
            logger.info(f"Using cached prompt: {template_file}")
        ...
        if template_file in create_chain.prompt_cache:
            ...
            with create_chain.lock:
                ...
                # 再检查一次（防止并发重复加载）
                if (not cache_entry or cache_entry["last_modified"] != file_mtime):
                    logger.info(f"Reloading prompt template: {template_file}")
                    template = PromptTemplate.from_file(template_file, encoding="utf-8")
                    create_chain.prompt_cache[template_file] = {"template": template, "last_modified": file_mtime}
...
def create_graph(db_connection_pool: ConnectionPool, llm_chat, llm_embedding, tool_config: ToolConfig) -> StateGraph:
    ...
    # 添加工具节点，使用并行工具节点
    workflow.add_node("call_tools", ParallelToolNode(tool_config.get_tools(), max_workers=None))
    # 添加重写节点
